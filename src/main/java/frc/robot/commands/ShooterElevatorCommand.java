...
public class ShooterElevatorCommand extends Command {
  ...
  public void execute() {
    ...
    // if
    // (stick.getRawButton(Constants.CoralConstants.INITIATE_HUMAN_PLAYER_SEQUENCE))
    // {
    ...
    if (stick.getRawButton(8)) { // 8 is start button
      ...
      } else if (spd < -.1 && shooterElevator.getEncoder() < 10) { // 10 is the value that we might have to adjust, when
                                                                   // slow speed starts
      ...
      if (stick.getPOV() == 180) {
        if (shooterElevator
            .getEncoder() < (Constants.AlgaeConstants.EncoderBetweenL2AndL3 - Constants.AlgaeConstants.Delta)) {
              if (Math.abs(shooterElevator.getEncoder() - Constants.AlgaeConstants.EncoderBetweenL2AndL3) >= 30) { //if encoder distance is more than 30
                shooterElevator.moveElevator(0.40);
              }
              else if ((Math.abs(shooterElevator.getEncoder() - Constants.AlgaeConstants.EncoderBetweenL2AndL3) < 30) && (Math.abs(shooterElevator.getEncoder() - Constants.AlgaeConstants.EncoderBetweenL2AndL3) >= 20)) {
                shooterElevator.moveElevator(0.35);
              }
              else if ((Math.abs(shooterElevator.getEncoder() - Constants.AlgaeConstants.EncoderBetweenL2AndL3) < 20) && (Math.abs(shooterElevator.getEncoder() - Constants.AlgaeConstants.EncoderBetweenL2AndL3) >= 5)) {
                shooterElevator.moveElevator(0.25);
              }
              else if ((Math.abs(shooterElevator.getEncoder() - Constants.AlgaeConstants.EncoderBetweenL2AndL3) < 5) && (Math.abs(shooterElevator.getEncoder() - Constants.AlgaeConstants.EncoderBetweenL2AndL3) >= Constants.AlgaeConstants.Delta)) {
                shooterElevator.moveElevator(0.2);
              }
              else {
                shooterElevator.moveElevator(0.04);
              }
              //deleted if-statements above and uncomment below if it doesn't work
            //shooterElevator.moveElevator(0.2);
        } else if (shooterElevator
            .getEncoder() > (Constants.AlgaeConstants.EncoderBetweenL2AndL3 + Constants.AlgaeConstants.Delta)) {
              if (Math.abs(shooterElevator.getEncoder() - Constants.AlgaeConstants.EncoderBetweenL2AndL3) >= 30) { //if encoder distance is more than 30
                shooterElevator.moveElevator(-0.4);
              }
              else if ((Math.abs(shooterElevator.getEncoder() - Constants.AlgaeConstants.EncoderBetweenL2AndL3) < 30) && (Math.abs(shooterElevator.getEncoder() - Constants.AlgaeConstants.EncoderBetweenL2AndL3) >= 20)) {
                shooterElevator.moveElevator(-0.35);
              }
        ...
        if (shooterElevator
            .getEncoder() < (Constants.AlgaeConstants.EncoderBetweenL3AndL4 - Constants.AlgaeConstants.Delta)) {
              if (Math.abs(shooterElevator.getEncoder() - Constants.AlgaeConstants.EncoderBetweenL3AndL4) >= 30) { 
                shooterElevator.moveElevator(0.4); //if encoder distance is more than 30
              }
              else if ((Math.abs(shooterElevator.getEncoder() - Constants.AlgaeConstants.EncoderBetweenL3AndL4) < 30) && (Math.abs(shooterElevator.getEncoder() - Constants.AlgaeConstants.EncoderBetweenL3AndL4) >= 20)) {
                shooterElevator.moveElevator(0.35); //if encoder distance is between 20 and 30 (less than 30 and greater than 20)
              }
              else if ((Math.abs(shooterElevator.getEncoder() - Constants.AlgaeConstants.EncoderBetweenL3AndL4) < 20) && (Math.abs(shooterElevator.getEncoder() - Constants.AlgaeConstants.EncoderBetweenL3AndL4) >= 5)) {
                shooterElevator.moveElevator(0.3); //if encoder distance is between 10 and 20 (less than 20 and greater than 10)
              }
              else if ((Math.abs(shooterElevator.getEncoder() - Constants.AlgaeConstants.EncoderBetweenL3AndL4) < 5) && (Math.abs(shooterElevator.getEncoder() - Constants.AlgaeConstants.EncoderBetweenL3AndL4) >= Constants.AlgaeConstants.Delta)) {
                shooterElevator.moveElevator(0.2); //if encoder distance is between delta (0.5) and 10 (less than 10 and greater than delta)
              }
              else {
                shooterElevator.moveElevator(0.04);
              }
...
// if (shooterElevator.isTopLimitReached() ||
// shooterElevator.isSensorDetected(shooterElevator.coralSensor)) {
...
// double speed =
// -stick.getRawAxis(Constants.ElavatorConstants.MANUAL_CONTROL_AXIS);
...
// else if (speed < 0) {
// if (shooterElevator.isSensorDetected(shooterElevator.Level1Sensor) ||
// shooterElevator.isSensorDetected(shooterElevator.coralSensor)) {
// shooterElevator.stopElevator();
